...
const Settings: React.FC<SettingsProps> = ({ settings, onSettingsChange, profile, onProfileChange, goals, onGoalsChange }) => {
    ...
    const handlePasswordChange = async (e: React.FormEvent) => {
        ...
        if (newPassword.length < 6) { setPasswordError("6 caract├¿res minimum."); return; }
    ...
    return (
        <div className="space-y-8 pb-20">
            ...
            {/* 2. SECTION S├ëCURIT├ë - MOT DE PASSE */}
                            ...
                            {newPassword && (
                                <div className="px-1 pt-1">
                                    ...
                                    <p className="text-[10px] font-bold mt-1 text-slate-500 uppercase tracking-tighter">
                                        Complexit├® : {passwordStrength <= 1 ? 'Faible' : passwordStrength <= 2 ? 'Moyenne' : passwordStrength <= 3 ? 'Forte' : 'Excellente'}
                                    </p>
                                </div>
                            )}
                        </div>
