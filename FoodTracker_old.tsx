...
const FoodTracker: React.FC<FoodTrackerProps> = ({ logs, onAdd, onDelete, goals, isProMember, onUpgradeClick }) => {
  ...
  const handleGenerateRecipes = async () => {
    ...
    try {
      ...
      console.error(e);
      setError("Impossible de g├®n├®rer des recettes.");
    } finally {
    ...
    <div className="space-y-6 w-full max-w-full overflow-hidden">
      ...
      {isCameraOpen && (
        <div className="fixed inset-0 bg-slate-950 z-50 flex flex-col">
          ...
          <button onClick={closeCamera} className="absolute top-4 right-4 text-white bg-black/50 p-3 rounded-full z-10" aria-label="Fermer la cam├®ra"><IconX className="w-6 h-6" /></button>
            ...
            <div className="flex items-center gap-3">
              {hasSupport && (
                <button
                  ...
                  title={isListening ? "Arr├¬ter l'├®coute" : "Dicter le repas"}
                >
